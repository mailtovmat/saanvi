...
const DATA = {
  ...
  docs: [
    {name:"2 Teacher LORs", kind:"Materials", due:"2026-08-15", done:false, pct:0, note:"Time-consuming — request ASAP. Family plan asked-by was 15 Aug."},
    {name:"1 Counsellor LOR", kind:"Materials", due:"2026-08-15", done:false, pct:0, note:"Request early alongside teachers."},
    {name:"1–3 Optional LORs", kind:"Materials", due:null, done:false, pct:0, note:"Not all unis accept these — check the list."},
    {name:"Transcripts from school", kind:"Materials", due:null, done:false, pct:0, note:"Request ASAP. Needed well before first deadline."},
    {name:"10th and 11th boards (if applicable)", kind:"Materials", due:null, done:false, pct:0, note:"Scan and keep in one Drive folder."},
    {name:"Certificates", kind:"Materials", due:"2026-08-24", done:false, pct:0, note:"Some schools want these before writing LORs."},
    {name:"SAT Scores", kind:"Materials", due:"2026-10-15", done:false, pct:0, note:"Send from College Board 1–2 weeks before a deadline that requires official reports."},
    {name:"FAFSA and CSS Profile", kind:"Materials", due:"2026-10-01", done:false, pct:0, note:"Both open 1 Oct. File early for the aid package with the offer."},
    {name:"Common App personal statement", kind:"Essay", due:"2026-08-31", done:false, pct:0, note:"650-word hard cap. Family plan target 31 Aug. Prompt not yet chosen."},
    {name:"Resume / activities list", kind:"Record", due:"2026-08-24", done:false, pct:0, note:"Resume for recommender packets (24 Aug). Activities list 10×150 chars by 30 Sep."},
    {name:"SAT score report", kind:"Test", due:"2026-08-19", done:false, pct:0, note:"Total not recorded. Meeting notes say “SAT - 150” — incomplete. Do not invent a number."},
    {name:"SAT sitting 22 Aug", kind:"Test", due:"2026-08-22", done:false, pct:0, note:"Not confirmed for Saanvi. Verify registration."},
    {name:"Official transcript", kind:"Record", due:null, done:false, pct:0, note:"From counselor. Needed well before first deadline. Some schools also want grades typed into Common App / SRAR."},
    {name:"School profile document", kind:"Record", due:null, done:false, pct:0, note:"Ask in the same counselor email."},
    {name:"Predicted grades", kind:"Record", due:null, done:false, pct:0, note:"Subject teachers → counselor. TBD-VERIFY when they are issued."},
    {name:"10th & 11th board documents, scanned", kind:"Record", due:null, done:false, pct:0, note:"Keep in one Drive folder."},
    {name:"All certificates, scanned", kind:"Record", due:"2026-08-24", done:false, pct:0, note:"Some schools want these before writing LORs."},
    {name:"AP Calculus BC score report", kind:"Test", due:null, done:false, pct:0, note:"Meeting notes: score 4. Separate send from College Board. Year TBD-VERIFY."},
    {name:"Passport / CRBA / SSN card", kind:"Record", due:"2026-08-18", done:false, pct:0, note:"Confirm Saanvi has an SSN. Do not transcribe the number anywhere on this site."},
    {name:"SRAR / self-report platform credentials", kind:"Record", due:null, done:false, pct:0, note:"Arrives after submission. Watch inbox."},
    {name:"Mid-year report", kind:"Record", due:"2027-02-01", done:false, pct:0, note:"Sends after fall grades. Typical — verify per school."}
  ],
